...
def LoadNaukri(headless):
    ...
    driver = None
    try:
        driver = webdriver.Chrome(executable_path= r"C:\Users\Naveen\Downloads\chromedriver-win64\chromedriver-win64\chromedriver.exe")
    except:
...
def UpdateProfile(driver):
    try:
        # data-ga-track="spa-event|EditProfile|Basic Details|EditOpen
        mobXpath = "//*[contains(@data-ga-track, 'spa-event|EditProfile|Basic Details|EditOpen')]"
        profeditXpath = "//a[contains(text(), 'UPDATE PROFILE')] | //a[contains(text(), ' Snapshot')] | //a[contains(@href, 'profile') and contains(@href, 'home')]"
        saveXpath = "//button[@ type='submit'][@value='Save'] | //*[@id='saveBasicDetailsBtn']"
        editXpath = "//em[text()='Edit']"

        WaitTillElementPresent(driver, mobXpath, "XPATH", 20)
        profElement = GetElement(driver, mobXpath, locator="XPATH")
        driver.execute_script("arguments[0].click();", profElement)
        # profElement.click()
